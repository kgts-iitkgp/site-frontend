...
export const BlogCard: React.FC<BlogCardProps> = ({ id, title, author, date, content, image }) => {
  return (
    <Link href={`/${id}`}>
      <div className="relative max-w-xs bg-cardbg shadow-md overflow-hidden transform transition-all duration-300 hover:shadow-lg hover:shadow-accent1/50 border-2 border-accent2 w-full lg:max-w-[400px] mx-auto rounded-lg"> 
        <div className="p-4 bg-cardbg overflow-hidden">
          <Image
            src={image}
            alt={title}
            className="w-full h-48 object-cover transition-transform duration-500 transform hover:scale-105 hover:rotate-1"  // Added rounded-t-lg for top corners
          />
        </div>
